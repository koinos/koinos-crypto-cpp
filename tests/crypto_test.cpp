...
BOOST_AUTO_TEST_CASE( keccak_256_test )
{
  ...
  BOOST_CHECK_EQUAL( koinos::util::to_hex( eh.digest() ),
  ...
  BOOST_CHECK_EQUAL( koinos::util::to_hex( h1.digest() ),
  ...
  BOOST_CHECK_EQUAL( koinos::util::to_hex( h2.digest() ),
...
BOOST_AUTO_TEST_CASE( merkle )
{
  ...
  std::transform( std::begin( values ),
                  ...
                  []( const std::string& s )
                  {
                    return hash( multicodec::sha2_256, s );
                  } );
...
BOOST_AUTO_TEST_CASE( protocol_buffers_test )
{
  ...
  block_topology.set_previous(
    koinos::util::converter::as< std::string >( hash( multicodec::sha2_512, previous_str ) ) );
...
BOOST_AUTO_TEST_CASE( variadic_hash )
{
  ...
  auto mhash2 =
    hash( multicodec::ripemd_160, block_topology, std::string( "a quick brown fox jumps over the lazy dog" ), x );
...
BOOST_AUTO_TEST_CASE( vrf_tests )
{
  ...
  auto priv_key = private_key::regenerate( multihash(
    multicodec::sha2_256,
    koinos::util::from_hex< digest_type >( "0xc9afa9d845ba75166b5c215767b1d6934e50c3db36e89b127b8a622b120f6721"s ) ) );
  auto pub_key  = priv_key.get_public_key();
  ...
  pub_key = public_key::deserialize( koinos::util::from_hex< compressed_public_key >(
